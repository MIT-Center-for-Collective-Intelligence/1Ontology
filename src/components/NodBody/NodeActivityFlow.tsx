  ...
  "algorithms": [
    {
      ...
      "performance_model": "T = t_{S1} + t_{S2} + (SoilLevel <= 3 ? t_{S4} : t_{S5}) + max(t_{S7}, t_{S8}) + (RemainingRows * t_{S10}) + t_{S11}",
      "advantages": "All key steps are clearly laid out in a simple sequence, reducing confusion. The parallel cleaning of lavatories and overhead bins helps save some time while the rest of the process is straightforward.",
      "disadvantages": "Overall completion can still be slowed by the mostly sequential flow, especially if SoilLevel is high and deep vacuuming is needed across many rows. Coordination is simpler but does not fully exploit the possibility of more parallel tasks."
    },
    {
      ...
      "performance_model": "T = max( max(t_{S2}, t_{S3}), (NeedLavatories == true ? t_{S5} : t_{S6}), max(t_{S8}, (UncleanRows * t_{S10}), t_{S11}), t_{S12} )",
      "advantages": "Maximizes concurrent cleaning by dividing tasks among multiple teams. Critical areas can be tackled simultaneously, significantly reducing overall cleaning time under sufficient staffing.",
      "disadvantages": "Requires well-coordinated teams to avoid interfering with each other, potentially leading to complex scheduling. If staff or resources are limited, full parallelization might not be feasible, reducing its efficiency gains."
    },
    {
      ...
      "performance_model": "T = (HighPriorityZones > 0 ? (HighPriorityZones * t_{S3}) : 0) + max((SeatRows * t_{S6}), t_{S7}, t_{S8}) + t_{S9}",
      "advantages": "Ensures that the most critical or dirtiest areas receive immediate attention and a thorough cleaning. Parallelizing the regular zones can reduce total time once the priority zones are addressed.",
      "disadvantages": "If the majority of zones are deemed high priority, the initial sequential segment can become a bottleneck. This method also relies heavily on accurate priority assessment; misclassification can lead to inefficiencies."
    },
    {
      ...
      "performance_model": "T = t_{S1} + max(t_{S3}, t_{S4}, (LavStatus == 'dirty' ? t_{S6} : t_{S7})) + (CleanupIssues * (t_{S9} + (IssueResolved == true ? t_{S11} : t_{S12}))) + t_{S13}",
      "advantages": "Identifies problems early and iteratively fixes them, ensuring a high cleanliness standard. The parallel core cleaning tasks can speed up major efforts while dedicated loops allow re-checking until all issues are resolved.",
      "disadvantages": "Potentially more time-consuming if repeated inspections uncover many problems. Requires disciplined tracking of issues and resolutions, increasing coordination complexity among cleaning crews."
    }
  ]
};
...
const NodeActivityFlow: React.FC<NodeActivityFlowProps> = ({ 
  ...
  useEffect(() => {
    const fetchAlgorithms = async () => {
      setLoading(true);
      try {
        // Using placeholder data - TODO: implement fetching
        setAlgorithms(sampleAlgorithmsData.algorithms);
      } catch (error) {
              ...
              >
                {algorithms.map((algorithm, index) => (
                  <Tab 
                    ...
                    sx={{
                      color: isDarkMode 
                        ? (activeTab === index ? "#90CAF9" : "#ffffff") 
                        : (activeTab === index ? "#1976D2" : "#000000"),
                      '&.Mui-selected': {

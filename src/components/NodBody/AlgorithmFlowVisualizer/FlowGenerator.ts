...
export class FlowGenerator {
  private nodes: ReactFlowNode<NodeData>[] = [];
  private edges: Edge[] = [];
  private nodeId = 1;
  private joinId = 1000;
  ...
  public generateFlow(algorithm: IAlgorithm): FlowGenerationResult {
    ...
    this.assignPositions(algorithm, rootId, 0, -dimensions.height/2, null, true);
  ...
  private getNodeWidth(type: string): number {
    return this.nodeDimensions[type]?.width || this.nodeDimensions[NODE_TYPES.TASK].width;
  }
  ...
  private getNodeHeight(type: string): number {
    return this.nodeDimensions[type]?.height || this.nodeDimensions[NODE_TYPES.TASK].height;
  }
  ...
  private calculateDimensions(activity: any): SubtreeDimensions {
    ...
    switch (activity.type) {
      case NODE_TYPES.SEQUENTIAL:
        return this.calculateSequentialDimensions(activity, nodeWidth, nodeHeight);
      
      case NODE_TYPES.PARALLEL:
        return this.calculateParallelDimensions(activity, nodeWidth, nodeHeight);
      
      case NODE_TYPES.CONDITION:
        return this.calculateConditionDimensions(activity, nodeWidth, nodeHeight);
  ...
  private calculateParallelDimensions(
    ...
    for (const subActivity of activity.sub_activities!) {
      ...
      if (subActivity !== activity.sub_activities![activity.sub_activities!.length - 1]) {
  ...
  private calculateConditionDimensions(
    ...
    let trueWidth = 0, trueHeight = 0;
    let falseWidth = 0, falseHeight = 0;
    ...
    const totalWidth = trueWidth + falseWidth + this.spacing.branch + this.spacing.edgeBuffer;
    
    // Account for merge node
    const maxBranchHeight = Math.max(trueHeight, falseHeight);
    const totalHeight = nodeHeight + this.spacing.vertical + maxBranchHeight 
      + this.spacing.vertical + this.getNodeHeight(NODE_TYPES.MERGE);
  ...
  private optimizeEdgeRouting(): void {
    ...
    const nodePositionMap = new Map<string, {
      x: number;
      y: number;
      width: number;
      height: number;
    ...
    this.edges = this.edges.map(edge => {
      ...
      else if (sourceNode.type !== NODE_TYPES.CONDITION && 
               targetNode.type === 'default' && 
               targetNode.width === this.getNodeWidth(NODE_TYPES.MERGE)) {
        // Branch-to-merge edges
        newEdge.type = 'smoothstep';
      }
      else if (sourceNode.type === NODE_TYPES.LOOP && edge.sourceHandle === 'loop') {
        // Loop feedback edges
        newEdge.type = 'bezier';
        newEdge.style = {
          ...newEdge.style,
      ...
      else if (sourceNode.type === NODE_TYPES.PARALLEL || targetNode.type === NODE_TYPES.JOIN) {
  ...
  private addNode(
    ...
    this.nodes.push({
      ...
      data: { 
        ...
        hasSubActivities: activity.sub_activities && activity.sub_activities.length > 0,
        isConditionTrue: isConditionPath,
        variables: activity.variables,
        condition: activity.condition ? Object.keys(activity.condition)[0] : 
                  activity.loop_condition ? Object.keys(activity.loop_condition)[0] : undefined
      },
    });
  }
  ...
  private addEdge(
    ...
    if (isConditionPath === true) {
      ...
      this.edges.push({
        ...
        labelBgStyle: { fill: this.isDarkMode ? '#1e1e1e' : '#ffffff', fillOpacity: 0.7 },
      });
      ...
      this.edges.push({
        ...
        markerEnd: { type: MarkerType.ArrowClosed, color: this.getNodeColor(activityType) },
      });
    }
  }
  ...
  private layoutSequentialActivity(
    ...
    return { lastX, lastY: currentY - this.spacing.vertical, lastId: lastNodeId };
  }
  ...
  private layoutParallelActivity(
    ...
    const joinNodeId = this.createJoinNode(x, maxY + this.spacing.vertical, NODE_TYPES.PARALLEL);
  ...
  private layoutConditionActivity(
    ...
    if (trueActivity) {
      trueResult = this.assignPositions(trueActivity, parentId, trueX, nextY, true);
    }
    
    // Process the "false" branch
    let falseResult: NodePosition = { lastX: x, lastY: nextY, lastId: parentId };
    if (falseActivity) {
      falseResult = this.assignPositions(falseActivity, parentId, falseX, nextY, false);
    }
    ...
    // Create merge node
    const mergeNodeId = this.createJoinNode(x, maxY + this.spacing.vertical, NODE_TYPES.CONDITION);
    ...
    return { 
      lastX: x, 
      lastY: maxY + this.spacing.vertical + this.getNodeHeight(NODE_TYPES.MERGE), 
      lastId: mergeNodeId 
    };
  }
  ...
  private layoutLoopActivity(
    ...
    for (const subActivity of activity.sub_activities!) {
      const result = this.assignPositions(subActivity, lastSubNodeId, x, currentY);
    ...
    this.edges.push({
      ...
      markerEnd: { type: MarkerType.ArrowClosed, color: this.getNodeColor(NODE_TYPES.LOOP) },
    });

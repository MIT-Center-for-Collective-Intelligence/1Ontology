...
export const NodeImageManager: React.FC<NodeImageManagerProps> = ({
  ...
  const getImageDiffStatus = (image: NodeImage) => {
    ...
    const previousUrls = selectedDiffNode.previousValue?.map((img: NodeImage) => img.url) || [];
    const newUrls = selectedDiffNode.newValue?.map((img: NodeImage) => img.url) || [];
  ...
  const handleDeleteClick = async (image: NodeImage, event: React.MouseEvent) => {
  ...
  const handleUploadSelectedImages = async () => {
    try {
      ...
      alert('Sorry, your images could not be uploaded');
    }
  };
          ...
          <Box sx={{
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          ...(diffStatus && {
                            '&::after': {
                              content: `"${diffStatus === 'added' ? 'New' : 'Removed'}"`,
                              position: 'absolute',
                              top: -12,
                              right: 8,
                              backgroundColor: diffStatus === 'added' ? 'green' : '#f44336',
                              color: 'white',
                              padding: '2px 8px',
                              borderRadius: '12px',
                              fontSize: '12px',
                      ...
                      >
                        Uploaded by {image.uploadedBy.fName} {image.uploadedBy.lName}
                        <Typography variant='subtitle2'>
                          {dayjs(new Date(image.uploadedAt.toDate()))
                            .fromNow()
                            .includes("NaN")
                            ? "a few minutes ago"
                            : `${dayjs(new Date(image.uploadedAt.toDate())).fromNow()}`}
                        </Typography>
                      </Typography>
                    </Box>
                  );
            ...
            {displayImages.length > 0 && (
              <ImageViewerDialog
                ...
                onDeleteImage={!selectedDiffNode ? handleDeleteClick : undefined}
                isDiffView={!!selectedDiffNode}
                diffStatus={selectedDiffNode?.changeType === 'add images' ? 'added' :
                  selectedDiffNode?.changeType === 'remove images' ? 'removed' : null}
              />
            )}
          </Box>
            ...
            <Box>
              ...
              <Typography variant="subtitle2" sx={{ opacity: 0.7 }}>
                {dayjs(new Date(currentImage.uploadedAt.toDate()))
                  ...
                  .includes("NaN")
                  ? "a few minutes ago"
                  : `${dayjs(new Date(currentImage.uploadedAt.toDate())).fromNow()}`}
              </Typography>
            </Box>
          </Box>
        </Paper>

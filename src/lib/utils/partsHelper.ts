...
export const refreshPartsInheritance = (
...
): { [partId: string]: { inheritedFromTitle: string; inheritedFromId: string; directSource: string } } => {
  const currentInheritanceParts = node.inheritanceParts || {};
  
  const allGeneralizations = node.generalizations.flatMap(collection => collection.nodes);
  ...
  for (const generalization of allGeneralizations) {
    ...
    const parentInheritancePartIds = Object.keys(parentNode.inheritanceParts || {});
  ...
  const updatedInheritanceParts: { [partId: string]: { inheritedFromTitle: string; inheritedFromId: string; directSource: string } } = {};
...
export const propagatePartsChangeToSpecializations = async (
  ...
  try {
    ...
    const allSpecializations = nodeData.specializations?.flatMap(collection => collection.nodes) || [];
...
export const breakInheritanceAndCopyParts = async (
  ...
  try {
    ...
    const generalizationInheritanceParts = referencedNode.inheritanceParts || {};
    Object.entries(generalizationInheritanceParts).forEach(([partId, partInfo]) => {
    ...
    if (user) {
      await saveNewChangeLog(db, {
        ...
        fullNode: nodeData,
        changeDetails: {
          reason: 'Broke inheritance and copied inherited parts to inheritanceParts'
        },
        skillsFuture: false
      });
    }
...
export const getGeneralizationParts = (
  ...
  // Add direct parts
  if (generalizationNode.properties?.parts) {
    generalizationNode.properties.parts.forEach((collection: any) => {
  ...
  if (generalizationNode.inheritanceParts) {
    Object.keys(generalizationNode.inheritanceParts).forEach((partId: string) => {

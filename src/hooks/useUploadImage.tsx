  ...
  const uploadImage = ({ event, path, confirmatory, imageFileName }: UploadImageInput): Promise<string> =>
    new Promise((resolve, reject) => {
      try {
        ...
        const hasValidFormat = ["image/jpg", "image/jpeg", "image/gif", "image/png", "image/webp"].includes(image.type);
        if (!hasValidFormat) {
        ...
        task.on(
          ...
          async function complete() {
            ...
            setIsUploading(false);
            console.log(imageGeneratedUrl, "imageGeneratedUrl--imageGeneratedUrl")
